...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const nestedCustomers = flatCustomers.map((customer) => {
      ...
      return {
        ...customerDetails,
        paket: id_paket ? { id_paket, nama_paket, harga_paket, kecepatan_paket, comment_paket } : null,
...
router.post("/", authorize(["field", "operator", "admin", "super_admin"]), async (req, res) => {
  ...
  try {
    ...
    await req.dbPelanggan.query(
      ...
      [id, name, no_telepon, address, sn, olt, odp, port_odp, id_paket || null, id_mitra || null]
    );
